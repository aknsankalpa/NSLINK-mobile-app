...
class _RequestLeaveScreenState extends State<RequestLeaveScreen> {
  ...
  Future<void> _loadLecturerData() async {
    try {
      ...
      // Fetch lecturer data from addLecture collection
      final QuerySnapshot lecturerDoc = await _firestore
          .collection('addLecture')
          .where('lectureEmail', isEqualTo: userEmail)
          .get();

      if (lecturerDoc.docs.isNotEmpty) {
        final lecturerData =
            lecturerDoc.docs.first.data() as Map<String, dynamic>;

        setState(() {
          indexController.text = lecturerData['lectureIndex'] ?? '';
          nameController.text = lecturerData['lectureName'] ?? '';
          facultyController.text = lecturerData['lectureFaculty'] ?? '';
        });
      }
    } catch (e) {
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          filled: readOnly, // Add background color for read-only fields
