...
import 'package:nslink_new/lecture/add_slot.dart';
import 'package:nslink_new/lecture/lecturedashboard.dart'; // Add this import
...
class _LecturerDashboardState extends State<LecturerMeetingDashboard>
  ...
  Future<void> _fetchLecturerData() async {
    ...
    try {
      // Corrected collection name to match what's in Firestore (case sensitive)
      final bookingsQuery = await FirebaseFirestore.instance
          .collection(
            'lecturerBookings',
          ) // Changed from 'lecturerbookings' to 'lecturerBookings'
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _detailRow('Date', meeting['date'] ?? 'Not specified'),
                _detailRow('Time', meeting['time'] ?? 'Not specified'),
                _detailRow(
                  'Student',
                  meeting['studentName'] ?? 'Not specified',
                ),
                _detailRow('Message', meeting['message'] ?? 'No message'),
                if (meeting['hasAttachment'] == true)
                  _detailRow(
                    'Attachment Name',
                    meeting['attachmentname'] ?? 'No name',
                  ),
                _detailRow(
                  'Has Attachment',
                  meeting['hasAttachment'] == true ? 'Yes' : 'No',
                ),
              ],
            ),
          ),
          actions: [
            ...
            if (meeting['hasAttachment'] == true &&
                meeting['attachmentUrl'] != null &&
                meeting['attachmentUrl'].isNotEmpty)
              TextButton(
                onPressed: () {
                  // Handle the attachment viewing logic here
                  // You might want to open the URL in a browser or display the image/PDF
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Opening attachment...')),
                  );
                  // Implement attachment opening functionality
                },
