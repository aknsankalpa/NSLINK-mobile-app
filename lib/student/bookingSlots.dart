...
class _BookingFormPageState extends State<BookingFormPage> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // First check if the slot document exists before trying to update it
      ...
      final slotDoc = await slotDocRef.get();

      if (slotDoc.exists) {
        // Only update slot status if document exists
        await slotDocRef.update({'booked': true});
      } else {
        print('Warning: Slot document not found - skipping slot update');
        // You could show a warning here but continue with the booking
      }

      // Add booking to lecturerBookings collection
      await _firestore.collection('lecturerBookings').add({
        ...
        'time': '${_fromController.text} - ${_toController.text}',
